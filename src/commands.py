...
class CommandHandler:
    ...
    async def _cmd_lobotomize(self, message, sent_message) -> None:
        ...
        try:
            ...
            if deleted_count > 0:
                response = self._get_lobotomize_response_with_count(deleted_count)
                await sent_message.edit(content=response)

        except Exception as e:
            logger.error(f"Error executing lobotomize command: {e}")

    def _get_lobotomize_response_with_count(self, count: int) -> str:
        """Get a random witty response for the lobotomize command with message count"""
        responses = [
